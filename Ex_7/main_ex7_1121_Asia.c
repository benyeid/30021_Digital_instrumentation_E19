...
GPIO_MOSFET(){
 RCC->AHBENR |= RCC_AHBPeriph_GPIOD; // Enable clock for GPIO Port A
 // Set pin PA0 to input
 GPIOD->MODER &= ~(0x00000003 << (2 * 2)); // Clear mode register
 GPIOD->MODER |= (0x00000000 << (2 * 2)); // Set mode register (0x00 - Input, 0x01 - Output, 0x02 - Alternate Function, 0x03 - Analog in/out)
 GPIOD->PUPDR &= ~(0x00000003 << (2 * 2)); // Clear push/pull register
 GPIOD->PUPDR |= (0x00000002 << (2 * 2)); // Set push/pull register (0x00 -No pull, 0x01 - Pull-up, 0x02 - Pull-down)
...
//Setup the TIMx_CCMR1 register:
 ...
 TIM16->CCR1 = 78; //78; //for 1V //128 for 50%
 }
...
//Setup the TIMx_CCMR1 register:
 ...
 //Set duty cycle to 50% by writing the CCRx register
 TIM17->CCR1 = 78; //78; //for 1V //128 for 50%
...
int main(void)
{
  ...
  while(1)
  {
        ...
        float Vc1_1 = (vdda/4095)*Ch1;
        int dutyCykle1 = (78 * Vc1_1)/1;

        printf("Duty Cykle 1 = %2d \n", dutyCykle1);

        TIM16->CCR1 = dutyCykle1;

        uint16_t Ch2 = ADC_measure_PA(ADC_Channel_2);
        float Vc1_2 = (vdda/4095)*Ch2;
        int dutyCykle2 = (78 * Vc1_2)/1;
        printf("Duty Cykle 2 = %2d \n", dutyCykle2);
